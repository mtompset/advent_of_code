...
our $VERSION = '1.0';
Readonly my $WIDTH_OF_COLUMNS => 4;
...
my $number_of_stacks = ( ( length $board[0] ) + 1 ) / $WIDTH_OF_COLUMNS;
...
foreach my $row (@board) {
    for my $stack_number ( 0 .. $number_of_stacks - 1 ) {
        my $position = 1 + ($stack_number) * $WIDTH_OF_COLUMNS;
...
foreach my $instruction (@moves) {
    if ( $instruction =~ /move\s(\d\d*)\sfrom\s(\d\d*)\sto\s(\d\d*)/xslm ) {
        ...
        for my $move_number ( 1 .. $how_many ) {
            my $box = pop @{ $stacks[$from_stack_number] };
            push @{ $stacks[$to_stack_number] }, $box;
        }
    }
}
