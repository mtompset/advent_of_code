...
$end   = draw_board(%board);
%board = clear_processed(%board);
...
print 'TOTAL PROCESSABLE ROLLS: ', $initial - $end, "\n";
    ...
    foreach my $y ( 1 .. $height ) {
        foreach my $x ( 1 .. $width ) {
            ...
            if (   $neighbours_count < $MOVABLE
                && $original_board{$x_index}{$y_index} ne $EMPTY )
            {
...
sub clear_processed {
    ...
    foreach my $y ( 1 .. $height ) {
        foreach my $x ( 1 .. $width ) {
            my $y_index = $y - 1;
            my $x_index = $x - 1;
            if ( $original_board{$x_index}{$y_index} eq $PROCESSABLE ) {
                $new_board{$x_index}{$y_index} = $EMPTY;
            }
            else {
                $new_board{$x_index}{$y_index} =
                  $original_board{$x_index}{$y_index};
