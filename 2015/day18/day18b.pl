...
%board = corners_always_on(%board);

my $board_height = scalar keys %board;
my $board_width  = scalar keys %{ $board{0} };
print "Height: $board_height -- Width: $board_width\n";
draw_board(%board);
...
sub corners_always_on {
    my (%board_to_fix) = @_;

    my %fixed_board = %board_to_fix;
    my $height      = scalar keys %board_to_fix;
    my $width       = scalar keys %{ $board_to_fix{0} };
    $fixed_board{0}{0}                        = $ON;
    $fixed_board{0}{ $height - 1 }            = $ON;
    $fixed_board{ $width - 1 }{0}             = $ON;
    $fixed_board{ $width - 1 }{ $height - 1 } = $ON;
    return %fixed_board;
}
